...
describe('CartService', () => {
  ...
  describe('addProduct', () => {
    it('should throw BadRequestException if quantity <= 0', async () => {
      await expect(service.addProduct('user1', 'prod1', 0)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw NotFoundException if product does not exist', async () => {
      ...
      await expect(service.addProduct('user1', 'prod1', 2)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should increment quantity if product is already in cart', async () => {
      ...
      const cartWithSave = {
        userId: 'user1',
        items: [
          { productId: { toString: () => 'prod1' }, quantity: 1, price: 10 },
        ],
        total: 10,
        status: 'ACTIVE',
        save: jest.fn().mockResolvedValue(true),
  ...
  describe('removeProduct', () => {
    it('should remove the product from cart items and recalculate total', async () => {
      const cartWithSave = {
        userId: 'user1',
        items: [
          { productId: { toString: () => 'prod1' }, quantity: 2, price: 10 },
        ],
        total: 20,
        status: 'ACTIVE',
        save: jest.fn().mockResolvedValue(true),
