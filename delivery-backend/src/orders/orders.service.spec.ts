...
const mockOrder = {
    ...
    createdAt: new Date(),
    save: jest.fn().mockResolvedValue({ _id: mockOrderId, status: OrderStatus.PENDING }),
};
...
describe('OrdersService', () => {
    ...
    describe('createOrder', () => {
        ...
        it('debe lanzar BadRequestException si el carrito está vacío', async () => {
            mockCartService.getActiveCart.mockResolvedValue({ items: [], total: 0, save: jest.fn() });

            await expect(service.createOrder(mockUserId, 'Calle 123')).rejects.toThrow(
                BadRequestException,
            );
        });
    });
    ...
    describe('findMyOrders', () => {
        it('debe retornar pedidos paginados del usuario', async () => {
            ...
            const result = await service.findMyOrders(mockUserId, { page: 1, limit: 10 });
        ...
        it('debe filtrar por status si se proporciona', async () => {
            ...
            const result = await service.findMyOrders(mockUserId, { status: OrderStatus.CANCELLED });
    ...
    describe('findOne', () => {
        ...
        it('debe lanzar NotFoundException si el pedido no existe', async () => {
            ...
            await expect(service.findOne(mockOrderId, { role: 'CLIENT', userId: mockUserId }))
    ...
    describe('cancelOrderByClient', () => {
        ...
        it('debe lanzar BadRequestException si el pedido no está en PENDING', async () => {
            ...
            await expect(service.cancelOrderByClient(mockOrderId, mockUserId))
    ...
    describe('updateStatus (ADMIN)', () => {
        it('debe actualizar el estado con una transición válida', async () => {
            const order = { ...mockOrder, status: OrderStatus.PENDING, save: jest.fn().mockResolvedValue(true) };
            mockOrderModel.findById.mockResolvedValue(order);
        ...
        it('debe lanzar BadRequestException con una transición inválida', async () => {
            ...
            await expect(service.updateStatus(mockOrderId, OrderStatus.PENDING))
                .rejects.toThrow(BadRequestException);
        });
        ...
        it('debe lanzar NotFoundException si el pedido no existe', async () => {
            ...
            await expect(service.updateStatus(mockOrderId, OrderStatus.CONFIRMED))
                .rejects.toThrow(NotFoundException);
        });
    });
});
